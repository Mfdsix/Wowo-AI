...
import { getOrGenerateLevel } from "@/lib/curiosity";

export const dynamic = "force-dynamic";

// ─── Curiosity Engine: Depth level L1–L4 (PRD §9) ────────────
// POST /api/curiosity/:discoveryId/level   body: { level: 1-4 }
//   → return / generate konten level tersebut (cached di DB).
export async function POST(
  ...
  try {
    ...
    const levelRow = await getOrGenerateLevel(
      discovery,
      level,
      req.signal
    );
    ...
    return NextResponse.json({ error: `Level error: ${msg}` }, { status: 502 });
  }
}
