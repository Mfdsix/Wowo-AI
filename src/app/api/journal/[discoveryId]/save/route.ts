...
import { saveDiscovery, recordEvent } from "@/lib/journal";
import { getCodeFromCookies, SUPER_ADMIN_CODE } from "@/lib/auth";

export const dynamic = "force-dynamic";

// ─── Journal Engine: Save paper (§19 #9) ──────────────────────
// POST /api/journal/:discoveryId/save   body: { note?: string }
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ error: `Journal save error: ${msg}` }, { status: 502 });
  }
}
